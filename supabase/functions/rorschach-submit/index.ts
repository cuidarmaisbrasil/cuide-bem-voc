...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Rorschach runs alongside ECIG wave
    ...
    const { error } = await admin.from("rorschach_responses").insert({
    ...
    if (error) {
      console.error("rorschach-submit insert error", error);
      return j({ error: "insert_failed" }, 500);
    }

    return j({ ok: true });
  } catch (e) {
    console.error("rorschach-submit error", e);
